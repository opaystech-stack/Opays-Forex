...
import {
  ...
  Image as ImageIcon, Camera, MessageCircle
} from 'lucide-react';
...
export default function LoansPage({ mode, onOpenDebts }) {
  const t = useT();
  const {
  ...
  // Onglet actif : forcé par `mode` quand fourni (Clients en haut à droite,
  // Prêts dans le menu latéral). Sans `mode`, comportement historique à onglets.
  const [activeTab, setActiveTab] = useState(mode || 'loans'); // 'loans' | 'clients'
  const showToggle = !mode;
  ...
  return (
    <div>
      ...
      {/* Sub-tab toggle — masqué quand un mode dédié est imposé */}
      {showToggle && (
      ...
      {activeTab === 'loans' && (
        <div>
          ...
          {/* Accès Dettes & créances (ce que tu dois / ce qu'on te doit) */}
          {onOpenDebts && (
            <button
              type="button"
            ...
            >
              <Coins size={15} />
              <span>{t('debts.title')} — {t('debts.receivable')} / {t('debts.payable')}</span>
            </button>
          )}
        ...
        <div>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        {c.phone && (
                          <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap', marginTop: '2px' }}>
                            <Phone size={11} />
                            <span>{c.phone}</span>
                            <a
          ...
          {selectedCustomer && (
            <div className="modal-overlay" onClick={() => setSelectedCustomer(null)}>
              <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxHeight: '85vh', overflowY: 'auto', padding: '20px' }}>
                ...
                {/* Unified Customer Card (provides identity, WhatsApp relaunch button, operations total & list, and reminder logs) */}
                <CustomerCard customer={selectedCustomer} operations={getCustomerTransactions(selectedCustomer.id)} />

                {/* Loan history */}
                <div style={{ marginTop: '20px', padding: '16px', borderRadius: '17px', border: '1px solid var(--border-color)', backgroundColor: 'var(--bg-secondary, #f8fafc)' }}>
                  <h4 style={{
                    ...
                    getCustomerLoans(selectedCustomer.id).map(loan => {
                      ...
                      return (
                        <div key={loan.id} style={{
                          ...
                          backgroundColor: 'var(--card-bg, #ffffff)', marginBottom: '8px',
