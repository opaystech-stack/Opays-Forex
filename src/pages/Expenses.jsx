...
import { TrendingDown, CheckCircle2, AlertCircle, Trash2 } from 'lucide-react';
import { useT } from '../i18n';

export default function Expenses() {
  const { wallets, expenses, addExpense, deleteExpense, loading } = useApp();
  const t = useT();
  ...
  const handleDelete = async (expenseId) => {
    ...
    if (res.success) {
      setMessage({ type: 'success', text: t('expenses.delete_success') });
    } else {
      setMessage({ type: 'error', text: t('settings.rates_update_error') + res.error });
    }
  };
