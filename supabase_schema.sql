...
-- 3. Table des Transactions (Ledger) - AVEC STATUS BROUILLON
...
BEGIN
    ...
    -- Soustraire du portefeuille source
    UPDATE wallets 
    SET balance = balance - NEW.source_amount,
        updated_at = NOW()
    WHERE id = NEW.source_wallet_id;

    -- Ajouter au portefeuille destination
    UPDATE wallets 
